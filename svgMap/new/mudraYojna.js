function mudraYojna() {
    d3.queue()
        ...
        .await(function (error, mapData, yojnaData) {
            ...
            yojnaData.forEach(row => {
                var states = mapData.features.filter(
                    d => d.properties.ST_NM === row.state
                );
            ...
            tool_tip.html(function (d) {
                ...
                <p>Total Amount Under Shishu Yojana: ${
                  d.properties.shishuAmt
                } crore</p>
                <p>Accounts Under Kishore Yojana: ${d.properties.kishoreAc}</p>
                <p>Total Amount Under Kishore Yojana: ${
                  d.properties.kishoreAmt
                } crore</p>
                <p>Accounts Under Tarun Yojana: ${d.properties.tarunAc}</p>
                <p>Total Amount Under Tarun Yojana: ${
                  d.properties.tarunAmt
                } crore</p>
                `;
            });
            /////
            ...
            <text x="395" y="42" class='nonselectable' font-style='italic' text-anchor='middle' font-size='1em' font-family='sans-serif' fill='black'>Amount Disbursed</text>\
            <text x="392" y="62" class='nonselectable' font-style='italic' text-anchor='middle' font-size='1em' font-family='sans-serif' fill='black'>Under Mudra Yojna</text>`
                )
                .selectAll(".state")
    ...
    function changeChart(d) {
        myChart.config.data.labels = mudraYojana[d].labels;
        myChart.config.data.datasets[0].data = mudraYojana[d].data;
